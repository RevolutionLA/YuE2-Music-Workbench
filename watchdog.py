...
def find_listener_pid() -> int | None:
    """返回占用 7863 的进程 PID（用于精准击杀假死进程）。匹配行尾端口，避免 :78630 误命中。"""
    try:
        out = subprocess.run(
        ...
        for line in out.splitlines():
            if "LISTENING" not in line:
                continue
            parts = line.split()
            if len(parts) >= 2 and parts[1].rstrip().endswith(":7863"):
                return int(parts[-1])
    except Exception:
        pass
    return None
...
def main() -> None:
    ...
    try:
        if pid_file.is_file():
            ...
            if old and old != _current_pid():
                subprocess.run(["tasklist", "/FI", f"PID eq {old}"],
                               capture_output=True, text=True, timeout=15,
                               creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
                # tasklist 找不到该 PID 时输出不含 "PID"（表头除外），以此判断存活
    ...
    fails = 0
    cooldown_until = 0.0
    log("看门狗启动：监测 :7863 假死（连续 %d 次探测失败即重启）" % FAIL_THRESHOLD)
    while True:
