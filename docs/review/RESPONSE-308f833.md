...
## 九、第四轮：授权重启后的现场冒烟（2026-09-26 22:14–22:47）

裁定把发布条件写成"生效与诚实性以**一次授权重启**和三处报告修订为代价"。用户 09-26 明确解禁
（"现在 Yue2 没有任务在跑，你可以随时启停"），本轮补齐了那次重启，并把重启换来的现场证据落成整改。

### 九-1 冒烟通过项（网关 PID 换过之后实测，非静态推断）

| 冒烟项 | 现场结果 |
|---|---|
| 唯一 ID + 队列归组 | `20260926_221709_2368a1f2` / `…_52d6f384` / `20260926_221715_f28816d5` 三条各异，同属 `queue_id=20260926_221709_775e65a4` |
| 追加排队不吞条目 | `total` 58 → 61，旧条目一条没掉 |
| 取消 | 第三条 pending → `cancelled`，`runtime/data/*.tmp` 零残留 |
| 输入闸门 | 超上限模板字段回 **400**（不再静默截断） |
| 本机守卫 | `Origin: http://evil.example` 回 **403** |
| 看门狗兜底拉起 | 22:45:46 `网关 :7863 无人监听，看门狗主动拉起` → 30s 后 `/api/health` 恢复 |

**没有通过的一项，如实记下**：整个冒烟期没有任何条目跑到 `done`。`audio.cpp /v1/tasks/run request failed:`（空详情）——6GB 卡当时被 `llama-server` 与另一项目的 python ASR 进程占掉 5737/6144 MiB。这是运行环境，不是本轮改动的功能缺陷，但"能出歌"这件事要等显存腾出来再验，不能拿上面这张表宣称。

### 九-2 冒烟换来的两条现场缺陷（均已修，均已复验）

| 缺陷 | 现场证据 | 处置 |
|---|---|---|
| **蓝军 N-13 的漏字问题只是表层**：`app.py` 状态清扫把"仍在计算的条目"写成"服务重启，任务中断"，而网关 PID 从头到尾没变过 | 22:17 用户点"停止"后，`20260926_221709_52d6f384` 在下一次轮询（十秒内）被代写成该文案；worker 随后才把真结局（`error 220.1s`，audio.cpp 失败）覆盖上去——证明这句指控既假又是临时的 | `app.py:1936` 新增 `_PROC_START_TS`；`:2176-2198` 清扫改两条收口：worker 存活且 `current` 指向该条则不动它；只有 `started_ts` 早于本进程启动才有资格说"服务重启"，否则如实写"队列已停止：该条目的计算线程未回写结果，请重跑这一条"。复验 22:46：单条进入 running 后点停止，12 次轮询约 36s 全程保持 running，误标次数 0（`TestSweepHonesty` 三只用例锁死） |
| **看门狗会杀掉正在干活的网关** | `watchdog.log`：21:40:18 击杀 PID 16136、21:50:07 击杀 PID 12440，两次都落在批量任务计算期间（用户那两首的 `sec` 停在 471.2 与 15735.4）；根因是 5s 探活超时对上满载 6GB 卡 | `watchdog.py` `HEALTH_TIMEOUT 5→15`、`FAIL_THRESHOLD 3→5`、`probe(url, timeout=None)`，达阈值后加 `DEEP_PROBE_TIMEOUT=90` 深探确认——回话即判"慢"不判"死"并清零计数，宁可多等不再错杀 |

### 九-3 自查新发现（非审计条目，本轮一并修）

| 项 | 判定与处置 |
|---|---|
| 看门狗 `spawn_gateway()` 用 `DEVNULL` 丢弃网关 stdout/stderr | **已修**：`_spawn_log()` 追加写 `runtime/data/logs/gateway.{out,err}.log`，超 5MB 轮转 `.old`，句柄在 `Popen` 后关闭。证据是 `_gateway.log` 停在 09-20 15:35——此后每次自愈重启都把现场抹了；22:45 新路径首次留下 `Application startup complete` 与引擎健康请求 |
| "从未见过的服务不判定假死"这条规则套在网关上是**死锁**：看门狗单独起来（或网关比它先死）时 `seen` 永远为 False，既不会拉起也不会击杀僵在端口上的进程，而代码注释写着"网关必须常驻，重启后立刻拉回" | **已修**：`main()` 开局对网关做三轮确认——无人监听则主动拉起，有人监听却不回话则 `seen=True` 纳入常规连败→深探→击杀流程。22:45 实测走的就是"无人监听→拉起"这一支 |
| `audiocpp.py:111-118` 的 `detail=f"… request failed: {exc}"` 对裸 httpx 异常渲染成空串 | **记账，本轮不动**：这是引擎侧 OOM / 进程死 / 连接拒绝三种情形在页面上无法区分的原因，值得改，但它属于"错误可读性"而不是行为缺陷，且需要重启引擎侧才能复现全部三种。复入口：下次能稳定复现显存打爆时，把 `exc.__class__.__name__` 与 `exc.request` 一并写进 detail |

### 九-4 D16 版本策略（裁定留给用户的那一项，用户已授权代定）

用户原话"第 16 版本是干嘛的？你来定就行，我不懂"——D16 是**报告条目编号**（第三方评审第 16 条），不是第 16 个版本；它问的是"这个项目以后怎么标版本"。裁定如下：

- 版本真源 = git **附注标签**，命名 `v主.次.修订`（语义化版本）；`v1.0`（→ `c400c92`）作为"首个开源版本"保留，不回溯改写历史。
- 本轮基线打 `v1.1.0`：新增（唯一 ID、和弦勾选、LICENSE、评审归档）+ 行为变更（清扫文案诚实化、看门狗判死加确认、网关输出落盘），无接口破坏，故进次版本。
- 新增 `CHANGELOG.md`（Keep a Changelog 格式，中文），顶部条目即"当前版本"的权威说法。
- `docs/PROMOTION.md:13`/`:80` 原先把 `v1.0` 写死进发帖文案——那是会过期的假信息，改为不钉版本号、并在文件头写明"版本号只从 CHANGELOG 顶部条目取"；README 中英文新增「版本与发版」章节含发版清单（回归全绿 + 重启后冒烟三件 + 文档同步 + 打 tag 推 tag）。
